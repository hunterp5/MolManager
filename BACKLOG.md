# MolManager — actionable backlog (desktop)
...
**Progress (latest):** **B3**, **C1.1** + **C1.3**, **C1.2** (AST custom calculator + optional `MOLMANAGER_CUSTOM_CALC_LEGACY_EVAL`), **C2.3**, **D2** (`molmanager/config.py` + README env table), informational **pip-audit** CI. Next: **B2**, **D1**, **E1**, or deeper **A2** logging pass.
...
- **A1.3** ~~Smoke test: `import molmanager` + `ChemicalTableApp()` construction without showing (or short-lived widget) in CI.~~
...
- **A2.1** ~~Configure `logging` in `molmanager.app:main` (level from env `MOLMANAGER_LOG_LEVEL`, default INFO).~~
...
- **B1.1** ~~`QRunnable` (or reuse thread pool) that takes compiled SMARTS + list of `(oid, smiles)` or serialized inputs, returns `set` of oids that match (or bitmask).~~ (`SubstructureFilterWorker` + `SubstructureFilterSignals` in `workers.py`; `MOLMANAGER_SUBSTRUCTURE_ASYNC_ROWS`.)
...
- **B3.1** ~~Document and enforce a **default max row** for `load_from_sql` (user override in dialog + hard ceiling).~~ (`MOLMANAGER_SQL_MAX_ROWS_HARD`, dialog spinbox cap, clamp in `load_from_sql`.)
...
- **B3.3** ~~User-facing warning when result set exceeds threshold before loading.~~ (Pre-load `COUNT(*)` when ≥ `MOLMANAGER_SQL_PRECOUNT_WARN`; post-load note when row count hits limit.)
...
- **C1.1** ~~Settings or env: `MOLMANAGER_DISABLE_CUSTOM_CALC=1` for locked-down deployments.~~ (Env + greyed menu action + dialog guard.)
- **C1.2** ~~Replace default ``eval`` with AST-limited numeric evaluation (`molmanager/safe_calc.py`); ``MOLMANAGER_CUSTOM_CALC_LEGACY_EVAL`` restores the old path.~~
...
- **D1.1** `molmanager/services/session_io.py` — build/parse session dict, CSV helpers (no Qt).
- **D1.2** `molmanager/services/structure_io.py` — SMILES/InChI → mol, field population from mol (used by mixins + workers).
...
- **D2.1** ~~`molmanager/config.py` + ``MolManagerConfig`` / ``load_config()``: thread pools, filter debounce, SQL caps/timeouts, log level, calc flags, worker parallelism.~~
...
- **E1.1** ~~Starter PyInstaller spec + documented build steps (`packaging/molmanager.spec`, `docs/PACKAGING.md`).~~ Full one-click installer still TBD.
