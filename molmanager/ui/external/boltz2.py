...
class Boltz2Dialog(QDialog):
    ...
    def _append_failed_to_start_diagnostics(self) -> None:
        ...
        which_msg = repr(which) if which else "not found on PATH for this molmanager process"
        lines = [
            ...
            f"[{stamp}][system] 3) If the file exists but still fails: antivirus / SmartScreen may block child processes — try an exclusion or run molmanager from that same terminal once.",
    ...
    def _on_proc_error(self, error: int) -> None:
        ...
        if int(error) == QProcess.FailedToStart:
            ...
            QMessageBox.information(
                ...
                "The boltz program could not be launched from molmanager.\n\n"
