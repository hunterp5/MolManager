...
def _ensure_cairosvg_importable() -> None:
    ...
    if existing is not None and getattr(existing, "_CHEMMANAGER_CAIROSVG_STUB", False):
        return
    ...
    try:
        ...
        stub._CHEMMANAGER_CAIROSVG_STUB = True
        ...
        with _quieter_pkasolver_dependency_loggers():
            ...
            def _emit(done: int, *, force: bool = False) -> None:
                ...
                if force or done >= tot or (now - prog_last) >= 0.12:
                    prog_last = now
                    try:
                        self.worker_signals.tool_progress.emit("pKa prediction…", min(done, tot), tot)
                    except Exception:
                        pass
            ...
            if use_mp:
                ...
                logger.debug(
                    ...
                    "CHEMMANAGER_PKA_PROCESS_WORKERS>1 to allow parallel workers when unique≥2)",
                    n_work,
                    n_unique,
                )
            _safe_emit(self.pka_signals, "finished", out)
