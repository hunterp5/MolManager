...
def bundled_bin_dir() -> Path:
    override = (os.environ.get("CHEMMANAGER_BUNDLE_DIR") or "").strip()
    if override:
...
def default_external_executable(tool: str) -> str:
    ...
    Set ``CHEMMANAGER_BUNDLE_DIR`` to point at a directory containing platform binaries.
    """
...
def static_asset_path(name: str) -> Path:
    """Path to a file under ``chemmanager/ui/static`` (e.g. ``3Dmol-min.js``)."""
