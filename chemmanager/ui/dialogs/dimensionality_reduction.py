...
class _DimensionReductionDialogBase(QDialog):
    ...
    def _on_run(self) -> None:
        ...
        if df.empty and not use_fp:
            QMessageBox.information(self, self.windowTitle(), "No rows in the current scope.")
            return
        ...
        if use_fp:
            ...
            if not features:
                QMessageBox.warning(
                    self, self.windowTitle(), "Select at least one numeric feature column."
                )
                return
        ...
        try:
            ...
            if self.parent_app is not None:
                ...
                self.parent_app.status_label.setText(
                    f"{self.windowTitle()}: rendered {n:,} point(s). Lasso or click to select table rows."
                )
        except Exception as exc:
            QMessageBox.warning(self, self.windowTitle(), f"Plot failed: {exc}")

    def _on_failed(self, message: str) -> None:
        self._job_running = False
        self.run_btn.setEnabled(True)
        self.summary_text.setPlainText("")
        QMessageBox.warning(self, self.windowTitle(), message or "Computation failed.")


class PCADialog(_DimensionReductionDialogBase):
...
class TSNEVisualizationDialog(_DimensionReductionDialogBase):
...
class UMAPVisualizationDialog(_DimensionReductionDialogBase):
