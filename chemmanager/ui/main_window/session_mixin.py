...
class SessionMixin:
    _SESSION_FORMAT = "chemmanager_session"
    _SESSION_VERSION = 1

    def new_session(self) -> None:
        """Launch a new ChemManager instance with nothing loaded."""
        try:
            subprocess.Popen([sys.executable, "-m", "chemmanager"], close_fds=True)
        except Exception as e:
            QMessageBox.warning(self, "New Session", str(e))

    def duplicate_session(self) -> None:
        """Launch a new ChemManager instance with the current table state."""
        try:
            path = self._write_session_bundle_file()
            subprocess.Popen([sys.executable, "-m", "chemmanager", "--load-session", path], close_fds=True)
        except Exception as e:
    ...
    def _write_session_csv(self) -> str:
        ...
        session_dir = os.path.join(tempfile.gettempdir(), "ChemManagerSessions")
    ...
    def _apply_session_document(self, doc: dict) -> None:
        if doc.get("format") != self._SESSION_FORMAT or int(doc.get("version", 0)) != self._SESSION_VERSION:
    ...
    def save_session_as(self) -> None:
        path, _ = QFileDialog.getSaveFileName(
            self, "Save Session", "", "ChemManager Session (*.cms);;JSON (*.json)"
        )
        if not path:
            return
        low = path.lower()
    ...
    def open_session_file(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            ...
            "Open Session",
            "",
            "ChemManager Session (*.cms *.json);;Legacy session CSV (*.csv);;All files (*.*)",
        )
        if not path:
            return
    ...
    def apply_saved_session_from_file(self, path: str) -> bool:
        try:
            ...
            return False
        if d.get("format") != self._SESSION_FORMAT or int(d.get("version", 0)) != self._SESSION_VERSION:
            QMessageBox.warning(self, "Open Session", "Not a ChemManager session file (expected .cms / version 1).")
            return False
        try:
